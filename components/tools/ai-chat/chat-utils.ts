...
export const useMessageHandler = (
 ...
 const processThemeRequest = useCallback(
  async (userMessage: string) => {
    console.log('[Theme] Starting theme request processing'); // Log 1
    setIsLoading(true);
    setIsThemeMode(true);

    try {
      console.log('[Theme] Making API request with prompt:', userMessage); // Log 2
      ...
      console.log('[Theme] API response status:', response.status); // Log 3

      if (!response.ok) {
        const errorText = await response.text();
        console.error('[Theme] API error response:', errorText); // Log 4
        throw new Error(`Failed to get theme response: ${response.status} ${errorText}`);
      }

      const data = await response.json();
      console.log('[Theme] API response data:', data); // Log 5

      // Extract JavaScript code from the response
      const jsCode = extractCodeFromMarkdown(data.response);
      console.log('[Theme] Extracted JS code:', jsCode); // Log 6
      ...
      console.log('[Theme] Executing theme code...'); // Log 7
      const changes = await executeThemeCode(jsCode);
      console.log('[Theme] Execution result (changes):', changes); // Log 8

      // Apply changes directly to DOM
      if (changes && Array.isArray(changes)) {
        console.log('[Theme] Applying changes to DOM...'); // Log 9
        applyThemeChanges(changes);
        ...
        console.log('[Theme] Change descriptions:', changeDescriptions); // Log 10
        ...
        console.log('[Theme] Changes saved to localStorage'); // Log 11

        const cleanContent = `**Theme Applied Successfully!**\n\nChanges made:\n${changeDescriptions
          .map((change) => `- ${change}`)
          .join("\n")}`;

        console.log('[Theme] Returning success response'); // Log 12
        return {
          content: cleanContent,
          structuredContent: null,
        };
      } else {
        console.log('[Theme] No changes array returned'); // Log 13
        return {
          content:
      ...
      console.error('[Theme] Processing error:', error); // Log 14
      return {
      ...
      console.log('[Theme] Cleaning up (setting loading states to false)'); // Log 15
      setIsLoading(false);
    ...
    setMessages([
      {
        type: "assistant",
        content:
          "👋 Hey! I'm your AI assistant for this portfolio. I was created by Akash to help you learn about his work and experience. What would you like to know?",
        timestamp: new Date(),
      },
    ]);
  } catch (error) {
